...
func (q *Queries) InsertSession(session *types.Session) *errors.ErrorTrace {
	...
	query := `
		INSERT INTO sessions (userid, user_agent, ip_address, is_short_lived, is_api)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING sessionid, created_at, last_seen;	
	`

	err := q.Tx.
		QueryRow(q.Context, query, session.UserId.UUID(), session.UserAgent, session.IpAddress, session.IsShortLived, session.IsApi).
